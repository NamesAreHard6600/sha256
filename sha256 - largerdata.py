...
class Sha256():
    def __init__(self, s_input): #Types are way off: self.constants: string of hex, self.words: integers, self.working_vars: string of hex
        ...
        s_input = [s_input[i*56:((i+1)*56)-1].strip() for i in range((len(s_input)//56)+1)]
        self.s_input = s_input
        print(self.s_input)
        self.b_input = [stringtobinary(s_input).split() for x in s_input]
...
hash_input = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz12345"
